...
public class WebmvcConfiguration extends WebMvcConfigurationSupport {
    ...
    private HandlerExceptionResolver getHandlerExceptionResolver(){
        HandlerExceptionResolver handlerExceptionResolver = new HandlerExceptionResolver(){
            ...
            public ModelAndView resolveException(HttpServletRequest request, HttpServletResponse response,
                ...
                Result result = getResuleByHeandleException(request, handler, e);
     ...
     * @return
     */
    private Result getResuleByHeandleException(HttpServletRequest request, Object handler, Exception e){
        //Result result = new Result();
        if (e instanceof ServiceException) {
            Result.failure(ResultCode.DATA_IS_WRONG,null).setMsg(e.getMessage());
            //result.setCode(RetCode.FAIL).setMsg(e.getMessage()).setData(null);
            return Result.failure(ResultCode.DATA_IS_WRONG,e.getMessage(),null);
        }
        if (e instanceof NoHandlerFoundException) {
            //result.setCode(RetCode.NOT_FOUND).setMsg("接口 [" + request.getRequestURI() + "] 不存在");
            return Result.failure(ResultCode.INTERFACE_NOT_FOUND,"接口 [" + request.getRequestURI() + "] 不存在",null);
        }
        //result.setCode(RetCode.INTERNAL_SERVER_ERROR).setMsg("接口 [" + request.getRequestURI() + "] 内部错误，请联系管理员");
        Result failure = Result.failure(ResultCode.INTERFACE_INNER_INVOKE_ERROR, "接口 [" + request.getRequestURI() + "] 内部错误，请联系管理员");
        String message;
...
//    }
    ...
    public void addResourceHandlers(ResourceHandlerRegistry registry) {
        //registry.addResourceHandler("/cms/broadcasts/*").addResourceLocations("D:\\javaProject\\springboot\\geek-framework\\src\\main\\resources\\static");
        registry.addResourceHandler("/static/**").addResourceLocations("classpath:/static/");
        //registry.addResourceHandler("/image/**").addResourceLocations("file:D:/work/douyin/");
